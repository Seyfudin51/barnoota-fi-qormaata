...
async function loadStudentHome() {
  ...
  if (!data || data.length === 0) {
    box.innerHTML =
      "<div class='lesson-card'><div><h3>📚 Barnoonni hin jiru</h3><p>Admin barnoota yeroo booda ni dabala.</p></div></div>";
    return;
  }
  ...
  box.innerHTML = data.map(lesson => `
    <div class="lesson-card">
      ...
      <div>
        ...
        <p>${escapeHTML(lesson.description || "Barnoota haaraa")}</p>
      </div>
...
async function loadExams() {
  ...
  if (!data || data.length === 0) {
    box.innerHTML =
      "<div class='exam-card'><h3>📝 Qormaanni hin jiru</h3><p>Admin qormaata yeroo booda ni uuma.</p></div>";
    return;
  }
  ...
  box.innerHTML = data.map(exam => `
    <div class="exam-card">
      ...
      <p>
        ${escapeHTML(exam.description || "Qormaata")}
      </p>
...
async function startExam(examId) {
  ...
  if (error) {
    alert("Gaaffiiwwan qormaataa argachuu hin dandeenye.");
    return;
  }
...
function showQuestion() {
  ...
  document.getElementById("optionsBox").innerHTML =
    options.map(([letter, text]) => `
      <button
        class="option ${selectedAnswer === letter ? "selected" : ""}"
  ...
  if (currentQuestionIndex === currentQuestions.length - 1) {
    next.textContent = "Qormaata Xumuri ✓";
  } else {
    next.textContent = "Itti Aanee →";
  }
}
...
async function nextQuestion() {
  ...
  if (!selectedAnswer) {
    alert("Mee deebii tokko filadhu.");
    return;
  }
...
async function finishExam() {
  ...
  currentQuestions.forEach((q, index) => {
    ...
    if (
      ...
      currentAnswers[index].toUpperCase() ===
...
function showScore(score, total, percentage) {
...
async function saveProfile() {
  ...
  if (error) {
    alert("Maqaa haaromsuun hin danda'amne.");
    return;
  }
  ...
  alert("Maqaan kee sirriitti haaromfame.");
}
...
async function loadAdminStudents() {
  ...
  box.innerHTML = data.map((student, i) => `
    <div class="admin-list-item">
      <b>${i + 1}. ${escapeHTML(student.name)}</b>
      <small>
        Code: ${escapeHTML(student.student_code)}
      </small>
    </div>
  `).join("");
}
...
async function loadAdminResults() {
  ...
  box.innerHTML = data.map(result => `
    <div class="admin-list-item result-item">
      ...
      <div>
        <b>
          ${escapeHTML(result.students?.name || "Barataa")}
        </b>

        <small>
          ${escapeHTML(result.exams?.title || "Qormaata")}
          <br>
          Code:
          ${escapeHTML(result.students?.student_code || "")}
        </small>
      </div>
...
async function createLesson() {
  ...
  const title =
    document.getElementById("lessonTitleInput").value.trim();

  const description =
    document.getElementById("lessonDescriptionInput").value.trim();

  const content =
    document.getElementById("lessonContentInput").value.trim();

  const message =
    document.getElementById("lessonCreateMessage");
...
async function createExam() {
  ...
  const description =
    document.getElementById("examDescriptionInput").value.trim();

  const message =
    document.getElementById("examCreateMessage");
...
async function createQuestion() {
  ...
  const optionA =
    document.getElementById("optionAInput").value.trim();

  const optionB =
    document.getElementById("optionBInput").value.trim();

  const optionC =
    document.getElementById("optionCInput").value.trim();

  const optionD =
    document.getElementById("optionDInput").value.trim();

  const correct =
    document.getElementById("correctAnswerInput").value;

  const message =
    document.getElementById("questionCreateMessage");
...
async function loadAdminQuestions() {
  ...
  box.innerHTML = data.map((q, index) => `
    <div class="admin-list-item">
      ...
      <b>${index + 1}. ${escapeHTML(q.question)}</b>

      <small>
        Qormaata:
        ${escapeHTML(q.exams?.title || "")}
        <br><br>

        A) ${escapeHTML(q.option_a)}<br>
        B) ${escapeHTML(q.option_b)}<br>
        C) ${escapeHTML(q.option_c)}<br>
        D) ${escapeHTML(q.option_d)}<br><br>

        Deebii sirrii:
        <b>${escapeHTML(q.correct_answer)}</b>
      </small>
...
async function openAdminPanel(id) {
  ...
  if (id === "adminStudentsPanel") {
    await loadAdminStudents();
  }

  if (id === "adminResultsPanel") {
    await loadAdminResults();
  }

  if (id === "adminLessonsPanel") {
    await loadAdminLessons();
  }

  if (id === "adminExamsPanel") {
    await loadAdminExams();
    await loadQuestionExamSelect();
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    .getElementById("studentRoleButton")
    .addEventListener(
      "click",
      openStudentLogin
    );
  ...
  document
    .getElementById("adminRoleButton")
    .addEventListener(
      "click",
      openAdminLogin
    );
  ...
  document
    .getElementById("studentBackButton")
  ...
  document
    .getElementById("quickExamButton")
  ...
  document
    .getElementById("lessonBackButton")
    .addEventListener("click", () => {
  ...
  document
    .getElementById("examListBackButton")
    .addEventListener("click", () => {
  ...
  document
    .getElementById("scoreHomeButton")
    .addEventListener("click", async () => {
  ...
  document
    .getElementById("studentLogoutButton")
    .addEventListener(
      "click",
      studentLogout
    );
  ...
  document
    .getElementById("profileNavExams")
    .addEventListener("click", async () => {
  ...
  document
    .getElementById("profileNavScore")
    .addEventListener("click", async () => {
  ...
  document
    .getElementById("profileNavProfile")
    .addEventListener("click", () => {
  ...
  document
    .getElementById("adminStudentsButton")
    .addEventListener(
      "click",
      () => openAdminPanel("adminStudentsPanel")
    );


  document
    .getElementById("adminResultsButton")
    .addEventListener(
      "click",
      () => openAdminPanel("adminResultsPanel")
    );


  document
    .getElementById("adminLessonsButton")
    .addEventListener(
      "click",
      () => openAdminPanel("adminLessonsPanel")
    );


  document
    .getElementById("adminExamsButton")
    .addEventListener(
      "click",
      () => openAdminPanel("adminExamsPanel")
    );
  ...
  document
    .getElementById("closeStudentsPanel")
    .addEventListener(
      "click",
      hideAdminPanels
    );
  ...
  document
    .getElementById("closeLessonsPanel")
    .addEventListener(
      "click",
      hideAdminPanels
    );
  ...
  document
    .getElementById("closeExamsPanel")
    .addEventListener(
      "click",
      hideAdminPanels
    );
  ...
  document
    .getElementById("createLessonButton")
    .addEventListener(
      "click",
      createLesson
    );
